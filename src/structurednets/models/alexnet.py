...
class AlexNet(VisionModel):
    def __init__(self, output_indices: list, use_gpu=True):
        self.model = alexnet(pretrained=True).eval()

        self.feature_model = deepcopy(self.model)
        self.feature_model.classifier = torch.nn.Sequential(self.feature_model.classifier[:-1])
        self.feature_model.eval()

        super().__init__(output_indices, use_gpu=use_gpu)

    def get_optimization_matrix(self):
        return self.model.classifier[6].weight[self.output_indices,:].T

    def get_bias(self):
        return self.model.classifier[6].bias[self.output_indices]

    def get_feature_extractor_nb_params(self):
        nb_params = get_nb_parameters_in_model(self.model.features)
        nb_params += get_nb_parameters_in_model(self.model.classifier[:6])
        return nb_params

if __name__ == "__main__":
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    net = AlexNet([1,2,100,654])
