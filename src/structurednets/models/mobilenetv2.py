...
class MobilenetV2(VisionModel):
    def __init__(self, output_indices: list, use_gpu=True):
        ...
        self.feature_model.classifier[1] = torch.nn.Identity()
        self.feature_model.eval()

        super().__init__(output_indices, use_gpu=use_gpu)

    def get_optimization_matrix(self):
        return self.model.classifier[1].weight[self.output_indices,:].T

    def get_bias(self):
        return self.model.classifier[1].bias[self.output_indices]
